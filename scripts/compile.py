...
def compile_file(root, file):
    print("compiling %s" % os.path.join(root, file))
    _fp=FileIO(os.path.join(root, file), 'r')
    _src=_fp.read()
    _fp.close()

    _js=__BRYTHON__.compile_python(_src,file);
    if _js is not None:
       _fp1=FileIO(os.path.join(root, file.replace('.py', '.pyj')), 'w')
       _fp1.write(_js)
       _fp1.close()
    else:
       print("error compiling %s" % os.path.join(root, file))
...
_files=['_random.py', 'random.py', 'heapq.py', 'collections.py', 'VFS_import.py']
